 ...
 * - DEV: Falls back to localhost:3001 (ORIGIN ONLY)
 */
...
const ORIGIN = RAW_API_BASE.replace(/\/+$/, "");

// ✅ Always append /api here (single source of truth)
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt < retries && isRetryable(error)) {
        ...
        console.warn(
          `[API] Retry ${attempt + 1}/${retries} after ${delay}ms for ${path}`
        );
        await sleep(delay);
        continue;
      }
